...
const Slider: React.FC<SliderProps> = (props) => {
  ...
  let activePlan = Number((props.value / props.totalValue).toFixed(2)); // 按下时的比例
    ...
    (e) => {
      if (isSlide) {
        if (props.vertical) {
          ...
          let setpValue =
            ~~((((props.totalValue * moveY) / document.body.offsetHeight) * 3) +
            activeValue.current); // 计算当前的秒数
          props.setValue(setpValue);
        } else {
          let moveX = e.clientX - signPosition.current;
          let stepValue =
            ~~((((props.totalValue * moveX) / document.body.offsetWidth) * 3) +
            activeValue.current); // 计算当前的秒数
          if (stepValue < 0) {
            stepValue = 0;
