...
const Recommend: React.FC<RecommendProps> = () => {
  ...
  const [privateContent, setPrivateContent] = useState<
    { id: number; picUrl: string; name: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    getBanner({ type: 0 }).then((res: any) => {
      // 加载轮播图时的图片位置数量信息
      let dirCopy = [{ name: 'start' }, { name: 'middle' }, { name: 'end' }];
  ...
  return (
    <div className='px-24 py-5 w-full flex-1 flex flex-col justify-start items-center overflow-y-scroll overflow-hidden'>
      {/* 轮播图 */}
      <SlideShow
        imgUrlList={bannerUrlList!}
        dir={dir}
        setDir={setDir}></SlideShow>
      {/* 推荐歌单 */}
      <TitleBar title='推荐歌单' />
      {status === 'loading' ? (
        <div>Loading...</div>
        ...
        <ul className='w-full mt-5 grid grid-cols-5 gap-x-5 gap-y-5'>
          {songMenuList.result.map((item: SongMenuType) => {
      ...
      <ul className='w-full mt-5 grid grid-cols-3 gap-x-12 gap-y-6'>
        {newSongList.map((item) => {
          return <NewSongItem songItem={item} key={item.id} />;
        })}
      </ul>
      {/* 推荐MV */}
      <TitleBar title='推荐MV' />
