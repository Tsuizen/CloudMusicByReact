...
const MvItem: React.FC<MvItemProps> = (props) => {
    ...
    <li className='flex flex-col justify-between cursor-pointer'>
      ...
      <div className='relative overflow-hidden'>
        ...
        <div className={
          ...
          <span className='text-sm text-white'>{props.mvItem.playCount}</span>
        </div>
      </div>
      {/* mv信息 */}
      <div className='w-52 mt-2 flex flex-col items-start justify-start'>
        <span className='truncate text-sm text-gray-500 w-52 hover:text-white'>{props.mvItem.name}</span>
        <span className='text-xs text-gray-500'>{props.mvItem.artistName}</span>
      </div>
    </li>
  );
};
