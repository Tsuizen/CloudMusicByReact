...
const NewSongItem: React.FC<NewSongItemProps> = (props) => {
  return (
    <li className='flex rounded-sm justify-start items-center cursor-pointer hover:bg-gray-500 transition duration-150'>
      ...
      <div className='flex flex-col flex-1 truncate justify-between ml-3 py-1 space-y-2'>
        <div className='text-xs truncate w-full'>
          ...
          <span className='truncate ml-1 text-gray-500'>
            {props.songItem.alias ? `(${props.songItem.alias})` : ''}
          </span>
        </div>
        <span className='text-xs text-gray-500'>{props.songItem.singer}</span>
      </div>
    </li>
  );
};
