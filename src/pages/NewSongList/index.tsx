...
const NewSongList: React.FC<NewSongListProps> = () => {
  ...
  return (
    <div className='w-full flex-1 px-24 py-5 overflow-hidden overflow-y-scroll'>
      ...
      <div className='flex justify-start items-center'>
        ...
        <button className='btn btn-sm bg-red-500 btn-ghost space-x-1 text-white hover:bg-red-500'>
          <Play theme='outline' size='20' fill='#ffffff' strokeWidth={2} />
          <span>播放全部</span>
        </button>
        <button className='btn btn-sm ml-3 space-x-1 bg-white text-gray-700 hover:bg-gray-200 border-gray-200 hover:border-gray-200'>
          <AddOne theme='outline' size='20' fill='#424242' strokeWidth={2} />
          <span>收藏全部</span>
        </button>
      </div>
      <ul className='mt-6'>
        {loading ? (
          ...
          newSongList.map((item, index) => {
            return <NewSongItem newSongItem={item} key={item.id}></NewSongItem>;
          })
        )}
      </ul>
    </div>
