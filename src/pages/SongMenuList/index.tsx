...
const SongMenuList: React.FC<SongMenuListProps> = () => {
  const [hotSongCatlist, setHotSongCatlist] = useState<
    { id: number; name: string }[]
  >([]);
  const [activeTag, setActiveTag] = useState<string>(''); // 当前活动的标签
  const [activeBoutiqueCover, setActiveBoutiqueCover] = useState<{
    name: string;
    coverImgUrl: string;
    text: string;
  }>({
  ...
  return (
    <div
      ref={scrollBoxRef}
      className='w-full flex-1 px-20 pb-10 pt-5 overflow-hidden overflow-y-scroll'>
      {/* 精品歌单封面 */}
      <BoutiqueCover boutiqueCover={activeBoutiqueCover} />
      {/* 歌单标签栏 */}
      <TabBar
        hotSongCatlist={hotSongCatlist}
      ...
      <ul className='relative w-full mt-5 grid grid-cols-5 gap-x-12 gap-y-5'>
        {isLoading ? (
          ...
          songList.map((item, index) => {
            return <SongMenuItem key={item.id} menuItem={item} />;
          })
        )}
      </ul>
      {/* 分页栏 */}
      {isLoading === false && (
        <PagingToolbar
          ...
          setPageNumber={setActivePageNumber}></PagingToolbar>
      )}
    </div>
  );
};
