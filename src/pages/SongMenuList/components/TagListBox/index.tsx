...
const TagListBox: React.FC<TagListBoxProps> = (props) => {
  const [songCatList, setSongCatList] = useState<
    { type: number; name: string; hot: boolean }[]
  >([]); // 分类标签列表
  ...
  return (
    <div
      ...
      className='flex flex-col w-4/6 bg-gray-100 rounded-md absolute top-10 z-50 text-gray-500'>
      {/* 全部歌单 */}
      <div className='w-full h-10 p-7  flex items-center border-b border-gray-200'>
      ...
      <div className='w-full flex flex-col text-sm'>
        {categories.map((tag) => {
          return (
            <div
              className='w-full p-5 flex justify-between items-start'
              key={tag.type}>
                    ...
                    return (
                      <li key={index}>
                        <div className='indicator'>
                          ...
                          <button
                            onClick={() => {
                              handleSelectTag(category.name);
                            }}
                            className='btn btn-xs text-gray-500 hover:text-white hover:bg-red-400 bg-opacity-0 border-gray-100 hover:border-gray-100'>
                            {category.name}
                          </button>
                        </div>
                      </li>
