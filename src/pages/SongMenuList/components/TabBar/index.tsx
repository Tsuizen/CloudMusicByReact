...
const TabBar: FunctionComponent<TabBarProps> = (props) => {
  ...
  return (
    <div className='relative flex justify-between items-center my-3'>
      {/* 歌单分类按钮 */}
      <button ref={btnRef} className='btn w-26 bordered btn-sm bg-white border-gray-300 hover:bg-gray-100' onClick={() => { setShowTagListBox(true) }}>
        <span className='text-gray-500 w-max overflow-hidden'>{props.tagName}</span>
        <Right theme="filled" size="20" fill="#a6a6a6" />
      </button>
      {/* 热门标签列表 */}
